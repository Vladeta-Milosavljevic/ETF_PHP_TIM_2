...
class Mentor extends BaseController
{
    ...
    public function prijava()
    {
        ...
        $test = $testProvera ?? '';
        if ($test) {
            return redirect()->to('mentor/prijava_azuriraj');
        } else {
            return view('mentor/prijava', $data);
        }
    }
    ...
    public function prijava_sacuvaj()
    {
        if ($this->validate([
            ...
            return redirect()->to('mentor/home')->with('message', 'Успешно сачувана пријава');
        } else {
    ...
    public function prijava_azuriraj()
    {
       ...
       $data['mentor'] = $mentorUpit;
        ...
        // tema
        $temaUpit = $this->temaModel->builder()->where('id_student', 1)
    ...
    public function prijava_azuriraj1()
    {
        $query = $this->user->builder()
            ->select('id, username')
            ->join('auth_groups_users', 'auth_groups_users.user_id=users.id')
    ...
    public function obrazlozenje()
    {
        ...
        $test = $testProvera ?? '';
        if ($test) {
            return redirect()->to('mentor/obrazlozenje_azuriraj');
        } else {
            return view('mentor/obrazlozenje', $data);
        }
    }
    ...
    public function obrazlozenje_sacuvaj()
    {
        if ($this->validate([
            ...
            return redirect()->to('mentor/home')->with('message', 'Успешно сачувано образложење');
        } else {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
    }
    ...
    public function biografija_sacuvaj()
    {
        if ($this->validate([
            ...
            return redirect()->to('mentor/home')->with('message', 'Успешно сачувана биографија');
        } else {
            return redirect()->back()->withInput()->with('errors', $this->validator->getErrors());
        }
    }
